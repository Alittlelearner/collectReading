...
export class StatsService {
  ...
  async recordRead(): Promise<void> {
    ...
    await db.runAsync(
      `INSERT INTO daily_stats (date, read_count, added_count, streak_eligible)
       VALUES (?, 1, 0, 1)
  ...
  async recordAddition(): Promise<void> {
    const db = await getDatabase();
    const today = new Date().toISOString().slice(0, 10);
    ...
    await db.runAsync(
      `INSERT INTO daily_stats (date, read_count, added_count, streak_eligible)
       VALUES (?, 0, 1, 0)
       ON CONFLICT(date) DO UPDATE SET added_count = added_count + 1`,
      today,
    );
  }
